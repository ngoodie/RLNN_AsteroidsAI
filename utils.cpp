...
void utils::GetVerticesFromCircle(const Circlef& circle, std::vector<Point2f>& vertices)
{

	for (int i = 0; i < 8; i++)
	{
		Vector2f direction = Vector2f{ cos(i * (float(M_PI) / 4)), sin(i * (float(M_PI) / 4)) };
		vertices.push_back(circle.center + direction * circle.radius);
	}
	//glBegin(GL_LINE_LOOP);
	//{
	//	for (auto& p : vertices)
